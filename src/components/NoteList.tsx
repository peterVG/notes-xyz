...
const NoteList: React.FC<NoteListProps> = ({ notes, onSelectNote, selectedNoteId, onTagClick }) => {
  if (notes.length === 0) {
    return <div className="p-4 text-center text-slate-500 dark:text-slate-400">No notes found.</div>;
  }
  
  const handleTagButtonClick = (e: React.MouseEvent, tagName: string) => {
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                    {note.tags.slice(0, 3).map(tag => (
                        <button
                            ...
                            className="px-2 py-0.5 text-xs text-slate-600 dark:text-slate-300 bg-slate-200 dark:bg-slate-600 rounded-full hover:bg-slate-300 dark:hover:bg-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        >
                            {tag}
                        </button>
                    ))}
